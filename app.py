...
with tab2:
    ...
    if 'df_matriz_global' in st.session_state:
        st.success("🟢 🧠 ¡MEMORIA ACTIVA! La distribución de las 8 AM está guardada. Puedes subir múltiples PDFs por plataforma.")
        usar_memoria = True
    else:
        st.error("🔴 ⚠️ ¡MEMORIA BORRADA! SUBE LOS 3 CSVs DE LA MAÑANA AQUÍ ABAJO para reconstruir la matemática exacta.")
        usar_memoria = False
        
    col_t2, col_s2, col_k2 = st.columns(3)
    with col_t2:
        if not usar_memoria: csv_t2 = st.file_uploader("CSV Temu", type=["csv"], key="ct2")
        pdf_t2 = st.file_uploader("PDFs Temu (Opcional)", type=["pdf"], key="pt2", accept_multiple_files=True)
    with col_s2:
        if not usar_memoria: csv_s2 = st.file_uploader("CSV Shein", type=["csv"], key="cs2")
        pdf_s2 = st.file_uploader("PDFs Shein", type=["pdf"], key="ps2", accept_multiple_files=True)
    with col_k2:
        if not usar_memoria: csv_k2 = st.file_uploader("CSV TikTok", type=["csv"], key="ck2")
        csv_jmx = st.file_uploader("CSV TikTok JMX", type=["csv"], key="cjmx")
        pdf_k2 = st.file_uploader("PDFs TikTok", type=["pdf"], key="pk2", accept_multiple_files=True)
    
    if not usar_memoria: f_base2 = st.file_uploader("BASE", type=["xlsx", "xlsm"], key="b2")

    if st.button("✂️ Cortar Guías", type="primary"):
        
        with st.spinner("Ensamblando múltiples PDFs y Reporte Global..."):
            if usar_memoria:
                ...
                if len(csvs2) < 3:
                    st.error("🚨 Sube los 3 CSVs (Temu, Shein, TikTok).")
                    st.stop()
            ...
            # --- MAPEO INTELIGENTE DE TRACKINGS (TIKTOK Y TEMU) ---
            # Esto soluciona que TEMU no encontraba los números iMile en el PDF
            mapa_pedidos_cruzados = {}
            for _, r in df_matriz[df_matriz['PLATAFORMA'].isin(['TIKTOK', 'TEMU'])].iterrows():
                p = str(r['PEDIDO']).strip()
                o = str(r['ORDER_ID']).replace(' ', '').strip()
                t = str(r['TRACKING_ID']).replace(' ', '').strip()
                if o and o != 'nan': mapa_pedidos_cruzados[o] = p
                if t and t != 'nan': mapa_pedidos_cruzados[t] = p
                
            if csv_jmx:
                df_jmx = procesar_csv(csv_jmx, detectar_plataforma_csv(csv_jmx)[1], 'TIKTOK')
                jmx_map = {}
                for _, r in df_jmx.iterrows():
                    o = str(r['ORDER_ID']).strip().replace(' ', '')
                    t = str(r['TRACKING_ID']).strip().replace(' ', '')
                    if o and t and o != 'nan' and t != 'nan': 
                        mapa_pedidos_cruzados[t] = o
                        jmx_map[o] = t
            ...
            # =========================================================
            # PROCESAMIENTO MÚLTIPLES PDF: TEMU (CON SÚPER ESCÁNER)
            # =========================================================
            if pdf_t2:
                for file_t in pdf_t2:
                    reader_te = PyPDF2.PdfReader(file_t)
                    po_actual_te = None
                    patron_pdf_te = r'(PO-\d{3}-\d+|JMX\d+|606\d+|UP[A-Z0-9]+)'
                    
                    for num_pagina, pagina in enumerate(reader_te.pages):
                        texto_original = pagina.extract_text() or ""
                        # Comprime todos los espacios invisibles para burlar el formato de PyPDF2
                        texto_limpio = re.sub(r'\s+', '', texto_original)
                        
                        matches = re.findall(patron_pdf_te, texto_original) + re.findall(patron_pdf_te, texto_limpio)
                        matches = list(set(matches)) # Quita duplicados
                        
                        if matches:
                            id_encontrado = None
                            for m in matches:
                                m_clean = str(m).strip()
                                if m_clean in mapa_pedidos_cruzados:
                                    id_encontrado = mapa_pedidos_cruzados[m_clean]
                                    break
                            
                            if id_encontrado:
                                po_actual_te = id_encontrado
            ...
            # =========================================================
            # PROCESAMIENTO MÚLTIPLES PDF: TIKTOK (CON SÚPER ESCÁNER)
            # =========================================================
            if pdf_k2: 
                temp_pages = {}
                po_actual_tk = None
                patron_pdf_tk = r'(JMX\d+)'
                for file_k in pdf_k2:
                    reader_tk = PyPDF2.PdfReader(file_k)
                    for pagina in reader_tk.pages:
                        texto_original = pagina.extract_text() or ""
                        texto_limpio = re.sub(r'\s+', '', texto_original)
                        
                        matches = re.findall(patron_pdf_tk, texto_original) + re.findall(patron_pdf_tk, texto_limpio)
            ...
            # =========================================================
            # PROCESAMIENTO MÚLTIPLES PDF: SHEIN
            # =========================================================
            if pdf_s2: 
                chunks_pdf_sh = []
                chunk_actual_sh = []
                for file_s in pdf_s2:
                    reader_sh = PyPDF2.PdfReader(file_s)
                    for pagina in reader_sh.pages:
                        texto_upper = (pagina.extract_text() or "").upper()
                        texto_limpio_upper = re.sub(r'\s+', '', texto_upper)
                        
                        es_declaracion = 'DECLARACIÓNDECONTENIDO' in texto_limpio_upper or 'DECLARACIÓN DE CONTENIDO' in texto_upper
                        tiene_indicadores = re.search(r'(JMX|GSH|J&T|TODOOR|D2D)', texto_limpio_upper)
            ...
            with zipfile.ZipFile(zip_buf, "a", zipfile.ZIP_DEFLATED, False) as zf:
                ...
                with pd.ExcelWriter(excel_buf, engine='xlsxwriter') as writer:
                    ...
                    df_global = df_matriz[df_matriz['PEDIDO'].isin(paginas_por_pedido.keys())].copy()
                    if not df_global.empty:
                        df_global[['ASIGNADO_A', 'PLATAFORMA', 'ORDER_ID', 'TRACKING_ID', 'SKU', 'Nombre Correcto', 'CANTIDAD', 'CATEGORIA']].to_excel(writer, sheet_name='🌐 REPORTE GLOBAL', index=False)
