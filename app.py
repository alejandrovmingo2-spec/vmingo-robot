...
tab1, tab2 = st.tabs(["🛒 FASE 1: Picking (8 AM)", "📦 FASE 2: Empaque (Guías)"])
...
with tab1:
    ...
    if f_t or f_s or f_k:
        ...
        if total_peds < 600:
            st.warning(f"⚠️ Detectamos solo **{total_peds}** pedidos totales.")
            ...
            with st.spinner("Procesando..."):
                ...
                with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                    if usar_ava == "SÍ":
                        ...
                        df_asig_ava = df_final[df_final['CATEGORIA'] == 'AVALANCHA'].groupby(['ASIGNADO_A', 'TRACKING_ID', 'SKU', 'Nombre Correcto'])['CANTIDAD'].sum().reset_index()
                    ...
                    for i, e in enumerate(emps):
                        df_e = df_final[(df_final['ASIGNADO_A'] == e) & (df_final['CATEGORIA'] == 'CARRITO')].groupby(
                            ['SKU','Nombre Correcto']
                        ).agg(CANTIDAD=('CANTIDAD', 'sum'), TIENE_TEMU=('PLATAFORMA', lambda x: 'TEMU' in x.values)).reset_index()
...
with tab2:
    ...
    if st.button("✂️ Cortar Guías", type="primary"):
        ...
        with st.spinner("Cortando..."):
            ...
            # Repetimos la lógica de conteo para decidir Avalancha
            ...
            mapa_pedidos_tiktok = {}
            df_tk = df_matriz[df_matriz['PLATAFORMA'] == 'TIKTOK']
            for _, r in df_tk.iterrows():
                ped, trk = str(r['PEDIDO']), str(r['TRACKING_ID'])
            ...
            if csv_jmx:
                ...
                for _, r in df_jmx.iterrows():
                    p, t = str(r['PEDIDO']), str(r['TRACKING_ID'])
                    if p and t: mapa_pedidos_tiktok[p] = p; mapa_pedidos_tiktok[t] = p
            ...
            st.subheader("📊 Diagnóstico de Guías Cortadas")
            ...
            with zipfile.ZipFile(zip_buf, "a", zipfile.ZIP_DEFLATED, False) as zf:
                ...
                with pd.ExcelWriter(excel_buf, engine='xlsxwriter') as writer:
                    ...
                    if not df_sin_pdf.empty:
                        df_sin_pdf[['PEDIDO', 'PLATAFORMA', 'TRACKING_ID', 'SKU', 'Nombre Correcto']].drop_duplicates(subset=['PEDIDO']).to_excel(writer, sheet_name='🚨 FALTAN EN PDF', index=False)
                    
                    if tot_f2 >= 600:
                        df_matriz[df_matriz['CATEGORIA']=='AVALANCHA'][['ASIGNADO_A','PEDIDO','TRACKING_ID','SKU','Nombre Correcto']].to_excel(writer, sheet_name='⚡ AVALANCHA GUIAS', index=False)
                    
                    for i, e in enumerate(emps2):
                        df_e = df_matriz[df_matriz['ASIGNADO_A'] == e].copy()
                        df_e = df_e[df_e['PEDIDO'].isin(paginas_por_pedido.keys())]
                        if not df_e.empty:
                            df_e[['PEDIDO','TRACKING_ID','PLATAFORMA','SKU','Nombre Correcto','CANTIDAD']].to_excel(writer, sheet_name=e, index=False)
